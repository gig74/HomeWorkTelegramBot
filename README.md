...
# Проект HomeWorkTelegramBot
# Проделанные изменения

За основу был взят fork предоставленного проекта java-bot-homework.
Из него за ненадобностью была удалена функциональность "/getHeroes". 

В переработанный телеграмм-бот были добавлены функциональности переводчика с установкой словаря перевода (простой режим "Эхо" по умолчанию), 
а также добавлена функциональность генератора английских фраз с переводом на русский язык.  

Для переводчика был взят libretranslate, развёрнутый в контейнере Docker с установленными двумя словарями(английским и русским). Все словари - это очень накладно. 
Используемая команда для развёртывания (потребовался VPN):  
 docker run -dit -p 5000:5000 -e LT_FRONTEND_LANGUAGE_SOURCE=en -e LT_FRONTEND_LANGUAGE_TARGET=ru -e LT_LOAD_ONLY=en,ru libretranslate/libretranslate  

Команда curl для проверки работоспособности переводчика libretranslate в Windows:  
 curl -X POST "http://localhost:5000/translate" -H "Content-Type: application/json" -d "{\"q\": \"Hello, world!\", \"source\": \"en\", \"target\": \"ru\"}"

По имеющемуся сделал следующие доработки:
- "вынес" имя и токен бота в переменные ОС (BOT_NAME и BOT_TOKEN). Их надо будет определить перед использованием бота.
- разделил обработку команд и текста для перевода (в соответствии с установленным режимом) в классе Bot
- сделал сохранение текущего режима раздельно для каждого чата (для целей обучение - оперативно в Map) 
- реализовал новые декларированные функциональности - установки режимов, перевода текста, выдачи сгенерированного текста с переводом.
- немного пофиксил метод sendPostRequest в BasicHttpService (для StringEntity явно указал кодировку, без этого сайт с русскими запросами не хотел работать).

# Оригинальное описание исходного проекта
...
1. Замените placeholder `YOUR_BOT_TOKEN` в классе `Bot` или в файле конфигурации на реальный токен, который вы получили. **ПРИМЕЧАНИЕ!!! Вынесено в переменную ОС BOT_NAME** 
2. Если бот будет взаимодействовать с пользователем через username, укажите это имя вместо `YOUR_BOT_USERNAME`. **ПРИМЕЧАНИЕ!!! Вынесено в переменную ОС BOT_TOKEN**
