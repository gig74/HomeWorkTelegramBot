...
- `configBotParameter` - служба для получения настроечных параметров имени бота и токена из переменных ОС. 
...
Метод `onUpdateReceived` проверяет наличие текстовых сообщений и обрабатывает команды(сообщения начинающиеся на "/"), вызывая соответствующие методы `commandService`, и тексты
(сообщения, начинающиеся на любой символ, кроме "/"), вызывая обработчик текста для текущего чата.
В случае возникновения ошибки при отправке сообщения зафиксирует ошибку с помощью `logger`.
